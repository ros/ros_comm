...
if [ "$3" == "--freq" ];then 
   ...
   NEW_TIME=$(time (./cmake-build-debug/devel/lib/mypakcage/rosbag-info -y -k "$1" --freq "$2" > "$RESULT") 2>&1)
else 
   OLD_TIME=$(time (rosbag info -y -k "$1" "$2" >  "$EXPECTED") 2>&1)
   sudo sync; sudo bash -c "echo 3 > /proc/sys/vm/drop_caches"
   NEW_TIME=$(time (./cmake-build-debug/devel/lib/mypakcage/rosbag-info -y -k "$1" "$2" > "$RESULT") 2>&1)
fi
