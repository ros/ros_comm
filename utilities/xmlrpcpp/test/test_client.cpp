...
TEST_F(MockSocketTest, writeRequest_large) {
  XmlRpcClientForTest a("localhost", 42);
  std::string largeString;
  largeString = std::string(size_t(__INT_MAX__) - 1, 'a');

  // Hack us into the correct initial state.
  a.setfd(7);
  a._connectionState = XmlRpcClientForTest::WRITE_REQUEST;
  a._request = largeString;

  // Check that request length is the correct size.
  ASSERT_EQ(largeString.length(), a._request.size());

  // Expect a write; write no bytes and return error.
  Expect_nbWrite(7, largeString, 0, false);
  Expect_close(7);
  Expect_getError(ECONNREFUSED);
  // Expect that writeRequest fails.
  EXPECT_FALSE(a.writeRequest());
...
TEST_F(MockSocketTest, writeRequest_overlyLarge) {
  XmlRpcClientForTest a("localhost", 42);
...
TEST_F(MockSocketTest, readResponse_oversize) {
  ...
  // Start with a pre-populated content-lenght that is within bounds
