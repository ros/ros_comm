...
ROS_UDS_EXT_ENABLE=on ${CPP_ADD_TWO_INTS_SERVER} $((SLEEP_TIME+2)) >/dev/null 2>&1  &
SERVICE_SERVERPID1=$!
sleep ${SLEEP_TIME}
...
ROS_UDS_EXT_ENABLE=on ${CPP_ADD_TWO_INTS_CLIENT} 1 2 __name:=c1 >/dev/null 2>&1  &
SERVICE_CLIENTPID1=$!
ROS_UDS_EXT_ENABLE=on ${CPP_ADD_TWO_INTS_CLIENT} 1 2 __name:=c2 >/dev/null 2>&1  &
SERVICE_CLIENTPID2=$!
sleep ${SLEEP_TIME}
